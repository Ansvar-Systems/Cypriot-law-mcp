...
Cypriot legal research means navigating [REDACTED] and the cyprus.gov.cy legislation portal across statutes in both Greek and English -- Cyprus has a unique legal system combining common law heritage with EU membership. Whether you're:
...
**Endpoint:** `https://mcp.ansvar.eu/law-cy/mcp`

| Client | How to Connect |
|--------|---------------|
| **Claude.ai** | Settings > Connectors > Add Integration > paste URL |
| **Claude Code** | `claude mcp add cypriot-law --transport http https://mcp.ansvar.eu/law-cy/mcp` |
...
```json
{
  "mcpServers": {
    "cypriot-law": {
      "type": "url",
      "url": "https://mcp.ansvar.eu/law-cy/mcp"
    }
  }
}
```
...
|----------|-------|---------|
| **Statutes** | 1,678 statutes | Comprehensive Cypriot legislation from [REDACTED] |
...
| **Daily Updates** | Automated | Freshness checks against [REDACTED] |

**Verified data only** -- every citation is validated against official sources ([REDACTED]). Zero LLM-generated content.
...
- All statute text is ingested from [REDACTED] and the official Cyprus legislation portal
...
```
[REDACTED] --> Parse --> SQLite --> FTS5 snippet() --> MCP response
               ^                        ^
...
|---------------------|-----------------|
| Search [REDACTED] by law number | Search by plain Greek or English: *"προστασία δεδομένων συγκατάθεση"* |
...
**Traditional:** Search [REDACTED] --> Navigate Greek-language pages --> Ctrl+F --> Cross-reference with EU directive --> Check EUR-Lex --> Repeat
...
- **[[REDACTED]](https://www.[REDACTED]/)** -- Official Cypriot law database maintained by the Cyprus Bar Association
...
| **Retrieval method** | Structured data from [REDACTED] and official government portal |
...
| Source | Check | Method |
|--------|-------|--------|
| **Statute amendments** | [REDACTED] date comparison | All statutes checked |
...
>
> Statute text is sourced from [REDACTED] and the official Cyprus legislation portal. However:
...
```bash
npm run ingest          # Ingest statutes from [REDACTED]
