#!/usr/bin/env tsx
/**
 * Database builder for Cypriot Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Cypriot Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Cypriot statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('source', 'https://www.cylaw.org');
