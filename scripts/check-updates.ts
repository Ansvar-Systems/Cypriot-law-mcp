#!/usr/bin/env tsx
/**
 * Cypriot Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.cylaw.org';
const PORTAL_NAME = 'CyLaw';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/cypriot-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Cypriot Law MCP — Data Freshness Check');
