/**
 * HTML parser for Cypriot legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Cypriot legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Cypriot statute.
 *
 ...
 *   </div>
 */
export function parseCypriotHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Cypriot acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Cypriot legal text.
 *
 * Cypriot definitions typically use patterns like:
