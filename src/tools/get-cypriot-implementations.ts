/**
 * get_cypriot_implementations — Find Cypriot statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetCypriotImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface CypriotImplementationResult {
  document_id: string;
...
export async function getCypriotImplementations(
  db: InstanceType<typeof Database>,
  input: GetCypriotImplementationsInput,
): Promise<ToolResponse<CypriotImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as CypriotImplementationResult[];
