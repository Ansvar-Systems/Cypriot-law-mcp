/**
 * search_eu_implementations — Search EU directives/regulations referenced by Cypriot legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_cypriot_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as cypriot_statute_count
  ...
  if (input.has_cypriot_implementation) {
    sql += ' HAVING cypriot_statute_count > 0';
  }
